...
// value 속성에 전달하고자 하는 데이터를 지정, 해당 데이터를 사용하고자 하는 자손 컴포넌트를 감싸면 어느 위치의 자손 컴포넌트인지 상관없이 해당 데이터에 접근이 가능하다.
// 사용법 :
// <컨텍스트명.Provider value = {전달할 데이터}>
//   <자손 컴포넌트/>
// </컨텍스트명.Provider>

// Context.Consumer : Provider로 감싸진 자손 컴포넌트 중 컨텍스트에 저장된 데이터를 사용하고자 할 경우 Consumer를 사용하여 저장된 데이터를 가져옴
// 사용법 :
// <컨텍스트명.Consumer>
//  {value => <컴포넌트 key = {value}/>}
//</컨텍스트명.Consumer>

//  Class.contextType : 클래스 컴포넌트에서 컨텍스트에 저장된 내용을 가져 올 경우 사용
// 사용법 :
// componentDidMount(){
...
// 개발자 도구에서 displayName으로 지정한 이름이 출력 됨
// 지정한이름.Provider, <지정한이름.Consumer>
// 사용법 :
// 컨텍스트명.displayName = '사용하고자 하는 이름';

// useContext : 리액트의 훅을 사용하기 위한 리액트 훅
// 사용법 :
// const value = useContext(컨텍스트명);

function App(props){
  // 자식 컴포넌트에 thema라는 이름으로 데이터를 전달
